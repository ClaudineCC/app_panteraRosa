...
const CategoriasGeral: React.FC = ({ navigation, route }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://localhost:3000/tbProduto/categoria/${categoriaId}`);
                ...
                Alert.alert("Erro", `Não foi possível buscar os produtos da categoria ${categoriaNome}.`);
            }
        };
        fetchData();
