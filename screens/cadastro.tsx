...
import { TextInput, Text, TouchableOpacity, View, ScrollView, SafeAreaView, StatusBar, StyleSheet, Linking, Alert, } from "react-native";
...
const Cadastro: React.FC = ({ navigation }) => {
    ...
    const handleSubmit = async () => {
        ...
        if (!nome || !nomeInteiro || !email || !cpf || !dataNascimento || !telefone || !senha || !confirmaSenha) {
        ...
        //acesso ao banco
        try {
            const response = await axios.post('http://localhost:3000/db_panteraRosa/tbPessoa', novoUsuario);
            if (response.status === 200) {
                Alert.alert('Sucesso', 'Cadastro enviado com sucesso!');
                navigation.navigate('Home');
            } else {
                Alert.alert('Erro', 'Ocorreu um erro ao enviar o cadastro. Tente novamente.');
            }
        } catch (error) {
        ...
        <SafeAreaView style={styles.safeArea}>
            ...
            <ScrollView contentContainerStyle={styles.scrollContainer} >
                ...
                <View style={styles.containerCadastro}>
                    ...
                    <TextInput style={styles.label} placeholder="Nome : "
                        value={nome}
                    ...
                    <TextInput style={styles.label} placeholder="Email: "
                        value={email}
                        onChangeText={setEmail}
                    />

                    <TextInputMask style={styles.label}
                        type={'cpf'}
                        value={cpf}
                        onChangeText={setCpf}
                        placeholder="CPF :"
                    ...
                    <View style={styles.senhaContainer}>
                        ...
                        <TouchableOpacity onPress={() => setSenhaVisivel(!senhaVisivel)}>
                            <Icon name={senhaVisivel ? "eye" : "eye-off"} size={24} color="gray" />
                        </TouchableOpacity>
                    </View>
                    ...
                    <View style={styles.senhaContainer}>
                        ...
                        <TouchableOpacity onPress={() => setConfirmaSenhaVisivel(!confirmaSenhaVisivel)}>
                            <Icon name={confirmaSenhaVisivel ? "eye" : "eye-off"} size={24} color="gray" />
                        </TouchableOpacity>
                    </View>
